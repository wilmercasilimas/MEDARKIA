  ...
  const obtenerCitas = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data?.citas)) {
        ...
        toast.error("No se pudieron cargar citas del doctor asignado.");
        setCitas([]);
        setTotalCitas(0);
      }
    } catch (error) {
      console.error("Error al obtener citas:", error);
      toast.error("Error al cargar citas del doctor asignado");
      setCitas([]);
      setTotalCitas(0);
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold">Citas del Doctor Asignado</h2>
